...
const SUBJECTS = ['Toán', 'Sinh học', 'Lịch sử', 'Vật lý', 'Hóa học']
    ...
    <div className="grid gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600">Xem danh sách lớp hoặc tạo lớp mới</p>
        </div>
        <div className="hidden md:flex items-center gap-3">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" />
            <input value={query} onChange={(e)=>setQuery(e.target.value)} className="w-64 rounded-2xl border border-slate-300 bg-white pl-9 pr-3 py-2 text-sm shadow-sm focus:border-brand-blue" placeholder="Tìm lớp học" />
          </div>
          ...
          <div className="inline-flex rounded-2xl bg-slate-100 p-1 text-sm">
            <button onClick={()=>setView('grid')} className={`rounded-xl px-3 py-1.5 flex items-center gap-1 ${view==='grid'?'bg-white text-slate-900 shadow-sm':'text-slate-600 hover:bg-white/60'}`}><LayoutGrid className="h-4 w-4"/> Lưới</button>
            <button onClick={()=>setView('table')} className={`rounded-xl px-3 py-1.5 flex items-center gap-1 ${view==='table'?'bg-white text-slate-900 shadow-sm':'text-slate-600 hover:bg-white/60'}`}><Table className="h-4 w-4"/> Bảng</button>
          </div>
        </div>

        <Modal open={open} onOpenChange={setOpen}>
          <ModalTrigger asChild>
            <Button variant="outline" className="gap-2"><Plus className="h-4 w-4"/> Tạo lớp mới</Button>
          </ModalTrigger>
          <ModalContent>
            <ModalHeader title={editingId ? 'Sửa lớp' : 'Tạo lớp học'} description={editingId ? undefined : 'Thiết lập lớp mới để mời học sinh'} />
        ...
        <Modal open={confirmOpen} onOpenChange={setConfirmOpen}>
          <ModalContent>
            ...
            <div className="grid gap-3">
              <div className="text-sm text-slate-700">{deleting.name ? `Lớp: ${deleting.name}` : ''}</div>
            ...
            <Card key={c.id}>
              ...
              <CardContent className="grid gap-3">
                <div className="flex items-center gap-3 text-sm text-slate-600">
                  ...
                  <Users className="h-4 w-4" /> {c.students} học sinh
                  <span className={`ml-auto rounded-full px-2 py-0.5 text-xs ${c.status==='active'?'bg-green-100 text-green-700':'bg-slate-200 text-slate-700'}`}>{c.status==='active'?'Đang hoạt động':'Tạm ngưng'}</span>
                </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Tất cả lớp</CardTitle>
        </CardHeader>
